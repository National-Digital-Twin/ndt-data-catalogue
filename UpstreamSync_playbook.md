...
```sh
git fetch
git pull origin intermediate-sync
git checkout sync/upstream-mirror-pr-22351868093 # <-- change name to suit
```

You will now need to fix up merge conflicts. Do this in your chosen way — I use VS Code to resolve the conflicts, then `git rebase --continue`.
